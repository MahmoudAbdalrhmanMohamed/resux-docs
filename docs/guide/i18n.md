...
For the exact package entry-point exports, signatures, types, defaults/fallback behavior, and SSR/client navigation boundaries, see the [i18n API reference](/reference/i18n).

The `resuxjs/i18n` entry point does **not** re-export its internal `shared` normalization/route/translation implementation helpers. Do not import undocumented internal file paths as if they were supported package APIs.
